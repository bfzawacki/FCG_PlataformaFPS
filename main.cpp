...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 308000 - Bruno Fialho", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        PushMatrix(model); // Guardamos a matriz do torso

        // Adicionando a cabeça
        model = model * Matrix_Translate(0.0f, 0.5f, 0.0f); // Translação para a posição da cabeça
        model = model // Aplicação das rotações da cabeça
...
// Adicionando a perna esquerda
    PushMatrix(model);
        ...
        PushMatrix(model);
            ...
            DrawCube(render_as_black_uniform);
        PopMatrix(model);
    // Adicionando o pé direito
        PushMatrix(model);
            model = model * Matrix_Translate(0.0f, -1.3f, 0.05f);
            model = model * Matrix_Scale(-0.25f, 0.15f, 0.4f);
            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
            DrawCube(render_as_black_uniform);
        PopMatrix(model);
    PopMatrix(model);

    // Adicionando a perna direita
    PushMatrix(model);
        model = model * Matrix_Translate(0.25f, -1.05f, 0.0f);
        PushMatrix(model);
            model = model * Matrix_Scale(0.3f, 0.6f, 0.2f);
    ...
    // Adicionando o braço esquerdo
PushMatrix(model); // Guardamos matriz model atual na pilha
    model = model * Matrix_Translate(0.55f, 0.0f, 0.0f); // Translação para o braço esquerdo (lado oposto)
    PushMatrix(model); // Guardamos matriz model atual na pilha
        model = model
            * Matrix_Rotate_Z(g_AngleZ)
            * Matrix_Rotate_Y(g_AngleY)
        ...
        PushMatrix(model);
            ...
            PushMatrix(model);
            model = model * Matrix_Translate(0.0f, -0.7f, 0.0f); // Translação para a posição da mão direita
        model = model // Aplicação das rotações da mão direita
              * Matrix_Rotate_Z(g_AngleZ)
              * Matrix_Rotate_Y(g_AngleY)
              * Matrix_Rotate_X(g_AngleX);
        PushMatrix(model);
            model = model * Matrix_Scale(0.2f, 0.2f, 0.1f); // Escala da mão direita
            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
